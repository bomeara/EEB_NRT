...
barplot(proportions, col=my.palette, ylab="Proportion of graduated students", xlab=gsub('.', " ", rownames(proportions)))
#legend(6.5,1, legend=gsub(" / ", ' or \\\n', rev(rownames(proportions))), col=rev(my.palette), fill=rev(my.palette), cex=1.3)
legend(6.5,1, legend=gsub("","", rev(rownames(proportions))), col=rev(my.palette), fill=rev(my.palette), cex=1.2)
dev.off()
